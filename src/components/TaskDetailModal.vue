...
function getProjectName(projectId: string | undefined) {
  ...
  return projects.value.find(p => p.id === projectId)?.title || t('tasks.without_project');
}
...
const dueDateModel = computed({
  get: () => editedTask.value.dueDate ? new Date(editedTask.value.dueDate).toISOString().split('T')[0] : '',
  set: (value: string) => {
...
<template>
  <div v-if="isOpen" class="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6">
    ...
    <!-- Modal Content -->
    <div class="relative w-full max-w-2xl bg-white dark:bg-gray-900 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-700 overflow-visible flex flex-col max-h-[90vh] animate-fade-in">
      ...
      <div class="flex items-center justify-between p-4 border-b border-gray-100 dark:border-gray-800">
        <div class="flex items-center gap-3">
          <button 
            ...
            :class="editedTask.completed ? 'text-green-500' : 'text-gray-400 hover:text-gray-600 dark:hover:text-gray-300'"
          >
            <CheckCircle2 v-if="editedTask.completed" class="w-6 h-6 fill-current" />
            <Circle v-else class="w-6 h-6" />
          </button>
          
          <span v-if="editedTask.completed" class="text-sm font-medium text-green-600 dark:text-green-400 bg-green-50 dark:bg-green-900/20 px-2 py-1 rounded-full">
            {{ t('tasks.completed') }}
          </span>
        </div>
      ...
      <div class="p-6 space-y-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-4 border-t border-gray-100 dark:border-gray-800">
          ...
          <!-- Project Dropdown -->
          <div class="space-y-1">
            <label class="text-xs font-medium text-gray-500 uppercase tracking-wider">{{ t('tasks.project') }}</label>
            <div 
              class="relative" 
            ...
            >
              ...
              >
                <div class="flex items-center gap-2">
                  ...
                  <span class="text-sm">{{ getProjectName(editedTask.projectId) }}</span>
                </div>
                <ChevronDown class="w-4 h-4 text-gray-400 transition-transform duration-200" :class="{ 'rotate-180': isProjectDropdownOpen }" />
              </button>
                ...
                >
                  <div class="p-1 max-h-60 overflow-y-auto">
                    ...
                    >
                      ...
                      <Check v-if="!editedTask.projectId" class="w-4 h-4 text-indigo-600" />
                    </button>
                    <button
                    ...
                    >
                      ...
                      <Check v-if="editedTask.projectId === project.id" class="w-4 h-4 text-indigo-600" />
                    </button>
                  </div>
                </div>
              </Transition>
          ...
          <!-- Priority Dropdown -->
          <div class="space-y-1">
            <label class="text-xs font-medium text-gray-500 uppercase tracking-wider">{{ t('tasks.priority') }}</label>
            <div 
              class="relative" 
            ...
            >
              ...
              >
                <div class="flex items-center gap-2">
                  ...
                  <span class="text-sm">{{ getPriorityLabel(editedTask.priority) }}</span>
                </div>
                <ChevronDown class="w-4 h-4 text-gray-400 transition-transform duration-200" :class="{ 'rotate-180': isPriorityDropdownOpen }" />
              </button>
                ...
                >
                  <div class="p-1">
                    ...
                    >
                      ...
                      <Check v-if="editedTask.priority === option.value" class="w-4 h-4 text-indigo-600" />
                    </button>
                  </div>
                </div>
              </Transition>
          ...
          <!-- Date Picker -->
          <div class="space-y-1">
            <label class="text-xs font-medium text-gray-500 uppercase tracking-wider">{{ t('tasks.due_date') }}</label>
            <DatePicker v-model="dueDateModel" />
          </div>

          <!-- Tags -->
          <div class="space-y-1">
            <label class="text-xs font-medium text-gray-500 uppercase tracking-wider">{{ t('tasks.tags') }}</label>
            <div class="flex flex-wrap gap-2 min-h-[42px] p-2 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg">
              <span 
              ...
              >
                ...
                >
                  <Hash class="w-3 h-3" />
                  {{ t('common.add') }}
                  <ChevronDown class="w-3 h-3 transition-transform duration-200" :class="{ 'rotate-180': isTagsDropdownOpen }" />
                </button>
                ...
                >
                  ...
                  >
                    <div class="p-1 max-h-40 overflow-y-auto">
                      <button
                        v-for="tag in tags.filter(t => !editedTask.tags?.includes(t.id))"
                        :key="tag.id"
                        @click="addTag(tag.id)"
                      ...
                      <div 
                        v-if="tags.filter(t => !editedTask.tags?.includes(t.id)).length === 0"
      ...
      <!-- Footer -->
      <div class="p-4 border-t border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-gray-900/50 flex justify-end gap-3">
        <button 
          @click="emit('close')"
