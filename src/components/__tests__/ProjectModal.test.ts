...
describe('AddProjectModal', () => {
  ...
  it('debe permitir seleccionar un icono', async () => {
    const wrapper = mountModal();
    const iconButtons = wrapper.findAll('button').filter(btn => 
      btn.find('svg').exists() && !btn.text().includes('Cancelar') && !btn.text().includes('Crear')
    );
  ...
  it('debe permitir seleccionar un color', async () => {
    const wrapper = mountModal();
    const colorButtons = wrapper.findAll('button').filter(btn => 
