...
let timer: number; // Removed NodeJS.Timeout for browser compatibility
...
const pageIcon = computed(() => {
  ...
  if (path.includes("/overview")) return "fa-chart-pie";
  if (path.includes("/users")) return "fa-users";
  if (path.includes("/projects")) return "fa-briefcase";
  if (path.includes("/tasks")) return "fa-list-check";
  if (path.includes("/database")) return "fa-database";
  if (path.includes("/profile")) return "fa-id-badge";
...
const pageTitle = computed(() => {
  ...
  if (path.includes("/overview")) return t("admin.overview");
  if (path.includes("/users")) return t("admin.users");
  if (path.includes("/projects")) return t("admin.projects");
  if (path.includes("/tasks")) return t("admin.tasks");
  if (path.includes("/database")) return t("admin.database");
  if (path.includes("/roles")) return t("admin.roles");
  if (path.includes("/audit")) return t("admin.audit");
  if (path.includes("/settings")) return t("admin.settings");
  if (path.includes("/profile")) return t("admin.profile");
        ...
        <router-link
          ...
          <div
            ...
            <img
              v-if="user.profile_image"
              :src="user.profile_image"
              class="w-40 h-40 rounded-full object-cover border-8 border-bg-dark shadow-2xl ring-4 ring-white/10" />
            <div
              v-else
              :class="[
                'w-40 h-40 rounded-full flex items-center justify-center text-6xl font-bold text-white shadow-2xl border-8 border-bg-dark ring-4 ring-white/10 bg-linear-to-br',
                theme.gradients.blob1.replace('blur', ''),
              ]">
              {{ user.username?.charAt(0).toUpperCase() }}
            </div>
            ...
            <!-- Online Status Dot -->
            <div
              class="absolute bottom-4 right-4 w-8 h-8 bg-emerald-500 border-4 border-bg-dark rounded-full shadow-lg"></div>
          </div>
        </router-link>
      </div>
    </div>
